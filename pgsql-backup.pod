...
Some of the configuration options in the rc file may be overridden on the
command line at run time. An alternative/specific rc file may also be passed
using the C<-c> option.

=over 4

=item -c /path/to/rcfile

Use the specified rc configuration file.

=item -d /path/to/output
